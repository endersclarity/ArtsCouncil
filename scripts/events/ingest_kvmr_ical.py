#!/usr/bin/env python3
"""
Ingest KVMR community radio iCal feed into canonical events JSON.

Fetches the public iCal subscription feed from KVMR's WordPress Events Calendar,
expands recurring events (RRULE) within the configured window, filters out
out-of-county events (Sacramento, etc.), and outputs events in the same canonical
schema as other ingest scripts.
"""
...
# Nevada County cities -- events outside these are filtered out
NC_CITIES = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert KVMR iCal feed into canonical events JSON."
    )
    parser.add_argument(
        "--ical-url",
        default=DEFAULT_ICAL_URL,
        help=f"KVMR iCal feed URL (default: {DEFAULT_ICAL_URL})",
    )
    parser.add_argument(
        "--output-file",
        type=Path,
...
def strip_uid_suffix(uid: str) -> str:
    """Strip domain suffix from UID if present (e.g., @kvmr.org)."""
    uid = uid.strip()
    if "@" in uid:
        uid = uid.split("@")[0]
    return uid
...
def make_event_id(uid: str, start_dt: datetime, title: str) -> str:
    """Generate a stable event_id with kvmr- prefix."""
    cleaned = strip_uid_suffix(uid)
    if cleaned:
        return f"kvmr-{cleaned}-{start_dt.strftime('%Y%m%d')}"
    slug = re.sub(r"[^a-z0-9]+", "-", (title or "").lower()).strip("-")
    if not slug:
        slug = "event"
    return f"kvmr-{start_dt.strftime('%Y%m%d%H%M')}-{slug[:30]}"


def infer_venue_city(location: str) -> str:
    """Infer city from KVMR LOCATION field by scanning for NC city names.

    Returns the properly-cased city name if found in NC_CITIES,
    or empty string if unknown/out-of-county.
    """
    ...
    for pattern, display_name in nc_city_display.items():
...
def infer_venue_name(location: str) -> str:
    """Extract venue name from KVMR location (first comma-separated part)."""
...
def parse_ical_events(
    ...
    """Parse iCal feed and return (events, recurring_expanded_count, skipped_out_of_county)."""
    tz = ZoneInfo(tz_name)
    now = datetime.now(tz)
    window_start = now
    ...
    for component in expanded:
        ...
        # City filtering: skip out-of-county events
        venue_city = infer_venue_city(location)
        if venue_city:
            # City was identified -- check if it's in NC
            if venue_city.lower() not in NC_CITIES:
                skipped_out_of_county += 1
                continue
        else:
            # City unknown -- keep event (benefit of the doubt for local events)
            pass
        ...
        event: dict[str, Any] = {
            ...
            "venue_city": venue_city if venue_city else "Nevada County",
            "source_type": "ical",
    ...
    return events, base_recurring, skipped_out_of_county
...
def main() -> int:
    ...
    events, recurring_count, skipped_count = parse_ical_events(
    ...
    print(
        f"Fetched {len(events)} events from KVMR ({recurring_count} recurring expanded, {skipped_count} out-of-county skipped)",
        file=sys.stderr,
    )
    print(f"[OK] KVMR iCal ingested", file=sys.stderr)
    ...
    print(f"  skipped_out_of_county={skipped_count}", file=sys.stderr)
    if events:
