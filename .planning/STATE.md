...
Next: (0) owner wants a /grill-me session on what else to improve — specifically measuring the live map against the Proposed Flows concepts (proposed-flows.html + docs/briefing-assets/flow-proposals/ images + docs/flow-proposal-gallery.html) and closing that gap; (1) owner reviews self-description merge locally (port 8013), then push to publish; (2) act on scripts/fullpass-link-findings.md — 88 broken venue links + crushnevadacity.com HIJACKED (gambling spam, remove link ASAP); (3) remaining placeholder data: 149 placeholder images, ~450 templated descriptions with no website, expired Google photo URLs (~1 in 8 broken) — candidates: Google Places API refresh or community photo submissions; (4) fold into formal milestone planning.
...
- Phase 09.1 inserted after Phase 9: Directory Audit Fixes (URGENT) — 3-agent Sonnet audit team (code-auditor, functional-tester, responsive-tester) found 4 blockers + 14 should-fix + 7 nice-to-have. Top blockers: XSS in explore-view innerHTML, null guards on getElementById, dead back button on hub explore, missing mobile map toggle. Also includes bookmark sync fix (event refreshAllBookmarkButtons) already applied.
