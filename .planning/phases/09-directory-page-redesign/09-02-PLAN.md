...
<task type="auto">
  ...
  **M15 — Back breadcrumb:** When a user arrives via deep link (has `?place=` in initial URL), add a breadcrumb above the expanded view. In the deep link handler, after `selectCategory(target.l)`, inject a breadcrumb element at the top of `#directoryHeaderArea`:
  ```javascript
