...
<task type="auto">
  ...
  **C5 — Events wiring:** This requires multiple changes:
  ...
  3. Add a helper function after the module declarations:
     ```javascript
     function getEventCountForAsset14d(assetIdx) {
       if (!matchedEventsByAsset || matchedEventsByAsset.size === 0) return 0;
       var matched = matchedEventsByAsset.get(assetIdx);
       if (!matched || !matched.length) return 0;
       var count = 0;
       var now = new Date();
       matched.forEach(function(evt) {
         var evtDate = eventsUtils.parseEventDate ? eventsUtils.parseEventDate(evt.date || evt.start) : null;
         if (evtDate) {
           var diff = (evtDate - now) / (1000 * 60 * 60 * 24);
           if (diff >= -1 && diff <= 14) count++;
         }
       });
       return count;
     }
  ...
  4. In `loadData()`, after `allData` is built and the subtitle is updated (after line 269), add an events fetch chain:
     ```javascript
