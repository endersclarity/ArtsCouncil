...
# NOTE: Gap-closure plans are exempt from requirement traceability.
# This plan addresses UAT validation failures, not roadmap requirements.
...
Preserve all other fields unchanged (coordinates, address, description, etc). These are the only 3 venues with this category mismatch — all other venues already use correct category names from config.

**CONTEXT CLARIFICATION:** The 02.2-CONTEXT.md decision "Bodhi Hive → Performing Arts" was informal shorthand for the venue type during planning discussion. The canonical config value (index-maplibre-config.js CATS object) is "Performance Spaces" — this is the exact string used by 26 other performance venues in data.json and required for category color lookup. The CONTEXT was describing the venue's PURPOSE (performing arts space), not the technical category field value. This task corrects the 3 venues added in 02.2-01 to match the canonical config.
